package com.kododake.hellbrowser.web
...
private fun handleCleartextIfNeeded(view: WebView, uri: Uri?, callbacks: BrowserCallbacks, onPageStart: Boolean = false): Boolean {
    ...
    if (com.kododake.hellbrowser.data.BrowserPreferences.isHostAllowedCleartext(view.context, host)) {
        return false
    }
    ...
    val allowHost = {
        view.context?.let { ctx ->
            ...
            if (hostToStore != null) com.kododake.hellbrowser.data.BrowserPreferences.addAllowedCleartextHost(ctx, hostToStore)
        }
        view.setTag(R.id.webview_allow_once_uri_tag, uri.toString())
        view.post { view.loadUrl(uri.toString()) }
        kotlin.Unit
...
private fun WebView.applyBrowserIdentity(profile: UserAgentProfile, desktop: Boolean) {
    ...
    val scalePercent = com.kododake.hellbrowser.data.BrowserPreferences.getGlobalScalePercent(context)
    if (desktop) {
        setInitialScale(0)
